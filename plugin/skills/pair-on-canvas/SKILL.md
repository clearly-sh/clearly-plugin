...
Don't guess. Drop a small **"🤖 Reading this as…"** card next to the spec restating the task + your assumptions, and tell them to correct it. One cheap round-trip here saves a wasted work cycle. (`canvas-act` → `canvas.create-node` text, placed in their `gaze`.)
...
```jsonc
// create once…
canvas-act { "compositionId":"c_…", "action":"canvas.create-node",
...
canvas-act { "compositionId":"c_…", "action":"canvas.update-nodes",
...
```
```jsonc
canvas-act { "compositionId":"c_…", "action":"canvas.add-diff",
